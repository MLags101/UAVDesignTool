...
class UAVDesignTool:
    ...
    def setup_main_ui(self, is_loaded=False):
        ...
        # Menu for save and parameter config import/export
        menubar = tk.Menu(self.root)
        filemenu = tk.Menu(menubar, tearoff=0)
        filemenu.add_command(label="Save Project", command=self.save_project)
        filemenu.add_separator()
        filemenu.add_command(label="Export Parameter Configuration", command=self.export_parameter_config)
        filemenu.add_command(label="Import Parameter Configuration", command=self.import_parameter_config)
    ...
    def export_parameter_config(self):
        """Export parameter configuration for all stages to a JSON file"""
        config_data = {
            "version": "1.0",
            "stages": {}
        }
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Success", f"Parameter configuration exported to:\n{file_path}")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to export parameter configuration:\n{e}")

    def import_parameter_config(self):
        """Import parameter configuration from a JSON file"""
        ...
        try:
            ...
            if "stages" not in config_data:
                messagebox.showerror("Invalid File", "The selected file is not a valid parameter configuration file.")
                return
            
            # Confirm with user before replacing
            if not askyesno("Confirm Import", 
            ...
            # Import configuration for each stage
            for stagename in ["Conceptual", "Preliminary", "Detail"]:
                if stagename in config_data["stages"]:
                    ...
                    # Restore parameter values that still exist in new config
                    ...
                    # Initialize parameters dict with preserved values
                    self.project_data["stages"][stagename]["parameters"] = preserved_params
                    
                    # Add empty values for new parameters
                    ...
                    # Refresh the parameter widgets if they exist
                    if "param_frame" in self.project_data["stages"][stagename]:
            ...
            messagebox.showerror("Error", f"Failed to import parameter configuration:\n{e}")
    ...
    def configure_parameters(self, stagename):
        ...
        def edit_param():
            ...
            if selected:
                ...
                if name:
                    default_val = "yes" if param["required"] else "no"
                    required = askyesno("Required", "Is this parameter required?", default=default_val)
                    old_name = param["name"]
    ...
    def generate_report(self):
        ...
        if self.doc_options["compile_pdf"].get() and shutil.which('pdflatex') is None:
            messagebox.showerror("LaTeX Not Found", "pdflatex is not installed or not in your PATH. Please install MiKTeX (https://miktex.org/) or TeX Live, and ensure pdflatex is accessible from the command line.")
            return
